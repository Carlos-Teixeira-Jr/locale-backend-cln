...
export class CreateProperty_Service {
  ...
  async createOne(createPropertyDto: CreatePropertyDto): Promise<any> {
    ...
    try {
      ...
      ownerActiveProperties = await this.propertyModel.find({owner: owner._id, isActive: true}).lean()
      ...
      if (
        ...
        if (selectedPlan.name === 'Free' && ownerActiveProperties.length > 0) {
          ...
          console.log("🚀 ~ CreateProperty_Service ~ createOne ~ deactivatepropertiesBody:", deactivatepropertiesBody)
  ...
  private async getUserAndOwner(
    ...
    if (!ownerExists) {
      ...
      if (ownerExists.plan.toString() !== selectedPlan._id.toString() || !ownerExists.plan) {
        owner = ownerExists
        ownerPreviousPlan = plans.find(e => e._id.toString() === owner.plan.toString())
        ...
        if (response.status >= 200 && response.status < 300) {
          ...
          try {
            ...
            if (typeof creditsLeft === 'number') {
              ...
              updatedOwner.adCredits = (selectedPlan.commonAd - 1) - ownerActiveProperties;
